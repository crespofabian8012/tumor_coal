...
void SimulateGenotype (TreeNode *treeRoot, std::vector<int> numberOfSitesWithKMutations, int numberVariableSites, int doISMhaploid, long int *seed,  std::vector<int> &DefaultModelSites, int numDefaultModelSites, std::vector<int> &AltModelSites, int numAltModelSites, long double  scaledTotalTreeLength , int &numISMmutations, int numFixedMutations, int numSNVmaternal, int doSimulateFixedNumMutations,  int alphabet,  std::vector<SiteStr> &allSites, int totalSampleSize, int  &numMU, long double  cumMij[4][4], long double  mutationRate, const gsl_rng *randomGsl, boost::mt19937* rngBoost)
{
    long double  cumBranchLength = 0;
    ...
    std::vector<long double> cumSumScaledBranchLengths(2*totalSampleSize-1);
    std::vector<long double> scaledBranchLengths(2*totalSampleSize-1);
    std::vector<int> numberMutationsPerBranch(2*totalSampleSize-1);
    double cumScaledTreeLength = 0.0;
    
    //std::cout << " totalSampleSize  "<< totalSampleSize<< std::endl;
    GetTreeDepthFirstSearchOrder (treeRoot,   mutationRate, nextAvailableIndex, depthFirstSearchTree, cumSumScaledBranchLengths, scaledBranchLengths,  cumScaledTreeLength, scaledTotalTreeLength);
    ...
    for (unsigned int i=1; i< numberOfSitesWithKMutations.size(); i++)
    {
        ...
        size_t end_sited_idx = cumulativeNumberSites+numberOfSitesWithKMutations[i]-1;
        end_sited_idx = std::min(end_sited_idx, allSites.size()-1);
        for (unsigned int j=start_site_idx; j<= end_sited_idx; j++)
        {
            ...
            for (unsigned int k=1; k<= i; k++){
                ...
                //std::cout << " scaledTotalTreeLength "<<scaledTotalTreeLength<< std::endl;
                ...
                numberMutationsPerBranch[idx]++;
                //std::cout << " random unif 0 to 1 "<<uniform<< std::endl;
                //std::cout << " idx label "<<depthFirstSearchTree[idx]->label<< std::endl;
                //std::cout << " idx cellName "<<depthFirstSearchTree[idx]->cellName<< std::endl;
                uniform = uniform * scaledTotalTreeLength;
                //std::cout << " random unif 0 to scaledTotalTreeLength "<< uniform<< std::endl;
                
                //std::cout << " numMU "<< numMU<< std::endl;
                added = false;
                
                PlaceMutation(depthFirstSearchTree[idx], j, allSites, seed, randomGsl, rngBoost);
                numMU++;
                //SimulateISMGenotypeforSite(treeRoot, 1, j, doISMhaploid, seed, scaledTotalTreeLength, allSites, numMU,cumMij, mutationRate, uniform, cumBranchLength,  ran, randomGsl,  rngBoost, added);
            }
            
        }
        cumulativeNumberSites+= numberOfSitesWithKMutations[i];
        if (cumulativeNumberSites>allSites.size())
            break;
    }
...
void SimulateISMDNAforSite (TreeNode *p, int genome, int site, int doISMhaploid, long int *seed, long double  totalTreeLength, std::vector<SiteStr> &allSites, int  &numMU, long double  cumMij[4][4],long double  mutationRate, long double  &uniform, long double  &cumBranchLength, long double  &ran, const gsl_rng *randomGsl, boost::mt19937* rngBoost )
{
        ...
        
    {
        ...
        SimulateISMDNAforSite (p->left.get(), genome, site, doISMhaploid, seed,  totalTreeLength, allSites, numMU,cumMij,  mutationRate, uniform, cumBranchLength,  ran,  randomGsl,  rngBoost);
        
        SimulateISMDNAforSite (p->right.get(), genome, site, doISMhaploid, seed, totalTreeLength, allSites, numMU,cumMij,  mutationRate, uniform, cumBranchLength,  ran,  randomGsl,  rngBoost);
...
void SimulateISMGenotypeforSite (TreeNode *p, int numberMutations, int site, int doISMhaploid, long int *seed, long double  scaledTotalTreeLength, std::vector<SiteStr> &allSites, int  &numMU, long double  cumMij[4][4],long double  mutationRate, long double  &uniform, long double  &cumBranchLength, long double  &ran, const gsl_rng *randomGsl, boost::mt19937* rngBoost, bool &added )
{
        ...
        
    {
            ...
            
        {
            ...
            {
                PlaceMutation(p, site, allSites, seed, randomGsl, rngBoost);
        ...
        SimulateISMGenotypeforSite (p->left.get(), numberMutations, site, doISMhaploid, seed,  scaledTotalTreeLength, allSites, numMU,cumMij,  mutationRate, uniform, cumBranchLength,  ran,  randomGsl,  rngBoost, added);
        
        SimulateISMGenotypeforSite (p->right.get(), numberMutations, site, doISMhaploid, seed, scaledTotalTreeLength, allSites, numMU,cumMij,  mutationRate, uniform, cumBranchLength,  ran,  randomGsl,  rngBoost, added);
...
void PlaceMutation(TreeNode *p, int site, std::vector<SiteStr> &allSites, long int *seed, const gsl_rng *randomGsl, boost::mt19937* rngBoost){
    int              anccell, maternal_ancstate, paternal_ancstate, ancestral_genotype, newstate, maternal_newstate, paternal_newstate, idx;
    long double one_over_6 = 1.0 / 6.0;
    long double  row_Qij[16] = { one_over_6, one_over_6, one_over_6, one_over_6, one_over_6, one_over_6 };
    ...
    idx = Random::ChooseUniformState(row_Qij, seed, true, randomGsl, rngBoost);
    
    newstate = GetNewGenotypeIdx ( ancestral_genotype,  idx);
...
void GetTreeDepthFirstSearchOrder (TreeNode *p, long double  mutationRate, int &nextAvailableIndex, std::vector<TreeNode *> &dfs, std::vector<long double> &cumSumScaledBranchLengths,  std::vector<long double> &scaledBranchLengths,  double &cumScaledTreeLength, long double scaledTotalTreeLength)
{
        ...
        
    {
        ...
        nextAvailableIndex++;
        GetTreeDepthFirstSearchOrder (p->left.get(),   mutationRate, nextAvailableIndex, dfs, cumSumScaledBranchLengths, scaledBranchLengths, cumScaledTreeLength, scaledTotalTreeLength);
        
        GetTreeDepthFirstSearchOrder (p->right.get(), mutationRate, nextAvailableIndex, dfs, cumSumScaledBranchLengths, scaledBranchLengths,  cumScaledTreeLength, scaledTotalTreeLength);
...
void SimulateISMforSite (TreeNode *p, int genome, int site, int doISMhaploid, long int *seed, long double  totalTreeLength, std::vector<SiteStr> &allSites, int  &numMU, long double  cumMij[4][4], long double  mutationRate, long double  &cumBranchLength, long double  &uniform, int &mutationAdded, const gsl_rng *randomGsl, boost::mt19937* rngBoost)
{
    ...
    if (p != NULL)
    {
        ...
        SimulateISMforSite (p->left.get(), genome, site, doISMhaploid, seed, totalTreeLength, allSites, numMU, cumMij, mutationRate, cumBranchLength, uniform, mutationAdded, randomGsl, rngBoost);
        SimulateISMforSite (p->right.get(), genome, site, doISMhaploid, seed,totalTreeLength, allSites, numMU,cumMij, mutationRate, cumBranchLength,  uniform, mutationAdded, randomGsl,  rngBoost);
...
int GetNewGenotypeIdx ( int indexCurrentGenotype, int selectedIdx)
{
    ...
    if (selectedIdx >=0 && selectedIdx < 6)
       idxNewGenotype = row[selectedIdx];
    
    return idxNewGenotype;
}
...
void SimulateFiniteDNAforSite (TreeNode *p, int genome, int site,std::vector<SiteStr> &allSites,  long int *seed, int rateVarAmongSites, long double  altModelMutationRate, int &numMU, int doJC, int doHKY, int doGTR, int doGTnR, long double  beta,  long double  kappa,  long double&  freqR,  long double&  freqY,  double  freq[4],  double  Root[],  double  Cijk[], const gsl_rng *randomGsl, boost::mt19937* rngBoost)
{
    ...
    if (p != NULL)
    {
        ...
        SimulateFiniteDNAforSite (p->left.get(),  genome, site,allSites, seed,  rateVarAmongSites, altModelMutationRate, numMU,  doJC,  doHKY, doGTR,  doGTnR,  beta,    kappa,   freqR,   freqY,   freq,  Root,  Cijk, randomGsl, rngBoost);
        SimulateFiniteDNAforSite (p->right.get(), genome, site,allSites, seed, rateVarAmongSites, altModelMutationRate, numMU,  doJC,  doHKY,  doGTR,  doGTnR,  beta,    kappa,   freqR,   freqY,   freq ,Root,  Cijk, randomGsl, rngBoost);
    }
}
...
void EvolveGenotypesOnTree (TreeNode *treeRoot, int totalSampleSize, std::vector<int> numberOfSitesWithKMutations, int numberVariableSites, long int *seed, int rateVarAmongSites, int numSites, std::vector<SiteStr> &allSites, int doGeneticSignatures, int alphaSites, int  propAltModelSites , int numDefaultModelSites, int numAltModelSites, std::vector<int> &DefaultModelSites, std::vector<int> &AltModelSites,  long double  totalTreeLength , int &numISMmutations, int numFixedMutations, int numSNVmaternal, int doSimulateFixedNumMutations,  int alphabet, int  &numMU, long double  cumMij[4][4], int altModel, long double  mutationRate, int doUserTree, int doJC, int doHKY, int doGTR, int doGTnR, long double  freqR, long double  freqY, long double  freqAG, long double  freqCT, double  titv, double  freq[4], double  Mij[4][4],   double  Root[],  double  Cijk[],
                            const gsl_rng *rngGsl,  boost::random::mt19937 * rngBoost  )
{
    ...
    SimulateGenotype (treeRoot,  numberOfSitesWithKMutations,  numberVariableSites, NO, seed,
                      DefaultModelSites,numDefaultModelSites, AltModelSites,  numAltModelSites,  scaledTotalTreeLength ,  numISMmutations, numFixedMutations, numSNVmaternal,  doSimulateFixedNumMutations,  alphabet, allSites,  totalSampleSize, numMU, cumMij, mutationRate, rngGsl,   rngBoost
                      );
...
void SimulateCNLOHforSite (TreeNode *p, int genome, int site,long int *seed,std::vector<SiteStr> &allSites,long double CNLOHrate, long double  &cumCNLOHbranchLength, long double totalTreeLength, long double mutationRate, int  &numCNLOH, const gsl_rng *rngGsl,  boost::random::mt19937 * rngBoost)
{
    ...
    if (p != NULL)
    {
        ...
        SimulateCNLOHforSite (p->left.get(), genome, site, seed,  allSites, CNLOHrate, cumCNLOHbranchLength,  totalTreeLength, mutationRate, numCNLOH, rngGsl,   rngBoost);
        SimulateCNLOHforSite (p->right.get(), genome, site, seed,  allSites, CNLOHrate, cumCNLOHbranchLength,  totalTreeLength, mutationRate, numCNLOH, rngGsl,   rngBoost);
    }
}
...
void SimulateDeletionforSite (TreeNode *p, int genome, int site, std::vector<SiteStr> &allSites, long double totalTreeLength, long double mutationRate,long double &cumDeletionBranchLength,  long double deletionRate, int &numDEL, long int *seed)
{
    ...
    if (p != NULL)
    {
        ...
        SimulateDeletionforSite (p->left.get(), genome, site,   allSites,  totalTreeLength, mutationRate, cumDeletionBranchLength, deletionRate,numDEL,seed);
        SimulateDeletionforSite (p->right.get(), genome, site,allSites,  totalTreeLength, mutationRate, cumDeletionBranchLength, deletionRate,numDEL, seed);
    }
}
...
void SequenceError (std::vector<TreeNode *> &treeTips,std::vector<SiteStr> &allSites, int alphabet, int numSites, int numCells,  double meanGenotypingError, int sampleSize, std::vector<int> numberOfSitesWithKSequencingErrors, double Eij[4][4], long int *seed, int &numberSeqErrorsAdded, const gsl_rng *rngGsl,  boost::random::mt19937 * rngBoost){
    ...
    for (j=1; j<numberOfSitesWithKSequencingErrors.size(); j++)
    {
        ...
        for (i=cumulativeNumberSites; i<(cumulativeNumberSites+numberOfSitesWithKSequencingErrors[j]); i++){
            //select j sequences randomly  out of 2*sampleSize
            std::vector<int> allMaternalPaternalIndexes(2*sampleSize);
            std::iota (std::begin(allMaternalPaternalIndexes), std::end(allMaternalPaternalIndexes), 0);
            ...
            for (k=0; k<j; k++){
                ...
                if (sample[k] % 2 == 0 ){
                    ...
                    if (tip->paternalSequence[j] != ADO && tip->paternalSequence[j] != DELETION){
                        probs = error_prob[tip->paternalSequence[j]];
                        newState =  Random::ChooseUniformState (probs, seed, true, rngGsl, rngBoost);
