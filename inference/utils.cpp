...
// taken from https://www.geeksforgeeks.org/find-closest-number-array/
double Utils::findClosest(std::vector<double> &arr, int n, double target)
{
    // Corner cases
    if (target <= arr[0])
        return arr[0];
...
// between these two.
double Utils::getClosest(double val1, double val2,
               double target)
{
