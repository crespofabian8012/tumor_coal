...
step.DeltaT <- function(current_deltaT,current_theta,current_time_std,current_coalescent_events, 
  ...
  log_alpha <- min(0, log(f_delta(proposal.delta)) + log(f_proposal) + log_f_proposal_coal_events+log(HR_q_delta) -log(f_current)- log_f_current_coal_events- log(f_delta(current_deltaT)) )
  #log_alpha <- min(0, logf(proposal) + log(HR_q) -  logf(current_value))
  print("stepDeltaT")
  print(log_alpha)
  if (log(runif(1)) < log_alpha)
...

{
  ...
  #alpha <- min(1, f(proposal) * HR_q / f(current_value)  )
  log_alpha <- min(0,  log(f_theta(proposal.theta))+log(f_proposal)+log_f_proposal_coal_events+log(HR_q_theta) -log(f_current)-log_f_current_coal_events -log(f_theta(current_theta)))
  #log_alpha <- min(0, logf(proposal) + log(HR_q) -  logf(current_value))
  print("stepTheta")
  print(log_alpha)
  if (log(runif(1)) < log_alpha)
...
delta=rexp(1,rate=lambdaExponentialPriorDelta)
theta =rexp(1,rate=lambdaExponentialPriorTheta)
coalescent_events_time_input=list(from )
#delta=200
...
list_res1 <- runChain( from,list(from ),   lambdaExponentialPriorDelta,lambdaExponentialPriorTheta, number.iterations,
                       f, q_transformed_multiplier, f_delta, f_theta, q_multiplier,  
                       HR_multiplier_bounded,HR_multiplier_unbounded)

list_res2 <- runChain( from,list(from ),  lambdaExponentialPriorDelta,lambdaExponentialPriorTheta, number.iterations,
                       f, q_transformed_multiplier, f_delta, f_theta, q_multiplier,  
                       HR_multiplier_bounded,HR_multiplier_unbounded )

list_res3 <- runChain( from,list(from ),  lambdaExponentialPriorDelta,lambdaExponentialPriorTheta, number.iterations,
                       f, q_transformed_multiplier, f_delta, f_theta, q_multiplier,  
                       HR_multiplier_bounded,HR_multiplier_unbounded )

list_res4 <- runChain( from, list(from ), lambdaExponentialPriorDelta,lambdaExponentialPriorTheta, number.iterations,
                       f, q_transformed_multiplier, f_delta, f_theta, q_multiplier,  
                       HR_multiplier_bounded,HR_multiplier_unbounded )
...
#thinned_res_deltaT_input_chain1=thin.chain.data(res_delta_chain1,number.iterations, percent_burn_in, thinning.delta_chain1)
...
number.iterations =500000
lambda1=100
path="/Users/faustofabiancrespofernandez/Downloads/tumor_coal_last_last/benchmarking/UsingData/DeltaT=100T=0.5"
...
#list_paths=c(chain1path, chain2path, chai3path )
...
plotPosterior(list_paths,number.iterations, 0.1,remove_burnIn_res_theta, max_thinning_theta,  "theta", path)

plotPosterior(list_paths,number.iterations, 0.1,remove_burnIn_res_delta,max_thinning_delta,  "deltaT", path)
