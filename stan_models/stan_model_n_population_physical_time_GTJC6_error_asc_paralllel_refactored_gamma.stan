...
parameters{
  ...
  vector<lower=0>[sum(number_internal_nodes)] independent_gamma_var;
...
transformed parameters {
  ...
  torigins_in_physical_time[N] = torigins_in_model_time[N]* pop_sizes_proportion[N];
   ...
   for(i in 1:N){
       ...
       sum_gamma_vars =  1 +  sum(independent_gamma_var[start_coal_idx:end_coal_idx]);
  
       //pi[start_idx:end_idx] = simplex_constrain( population_numbers[start_coal_idx:end_coal_idx]);

       pi[start_idx:end_idx] =  append_row(1,independent_gamma_var[start_coal_idx:end_coal_idx]) / sum_gamma_vars;
...
model{
  ...
  for (j in 1:N){ 
     ...
     //target += simplex_constrain_lj( population_numbers[start_coal_idx:end_coal_idx]);
      ...
      target += -(end_idx-start_idx+1) *log(sum_gamma_vars);
