functions{
  ...
  int find(real x, vector values){
     int i=1;
     int result= -1;
     while (i <= rows(values) && (abs(values[i] -x) >0.00001))
     {
          i=i+1;
     }
  ...
  int find_sorted(real x, vector sorted_values){
     int i=1;
     int result= -1;
     while (i <= rows(sorted_values)  && (abs(sorted_values[i] -x) >0.00001) && sorted_values[i]< x)
     {
          i=i+1;
     }
  ...
  real log_prob_father_population(int order, int index_oldest_population, real father_torigin, real father_delta,
   ...
   //print("previous_torigins_in_model_time=",previous_torigins_in_model_time);
    ...
    for(j in 1:rows(padded_coalescent_times)){
      ...
      if (abs(current_time - torigin)<0.00000001)
        return log_lik;
...
transformed data{
    ...
    print("additional_n_cols=",additional_n_cols);
    print("col_per_shards=",col_per_shards);
    ...
    for (k in 1:n_cores) {
         ...
         total_sample_size_topology_i[k, (3*total_sample_size):(3*total_sample_size+col_per_shards-1)]= site_weights[((k-1)*col_per_shards+1):(k*col_per_shards)];
...
model{
  ...
  row_vector[16] all_ones = to_row_vector(rep_vector(1, 16));
  row_vector[16] all_one_over_16 = to_row_vector(rep_vector(one_over_16, 16));
 ...
 for( k in 1:number_branches){
  ...
  log_lik_constant_site = sum(log(all_ones*genotype_internal_partials_asc[total_sample_size]));
  asc_correction = -1.0* sum_w * log(1.0-log_lik_constant_site);
