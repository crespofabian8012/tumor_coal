...
transformed data{
    ...
    for (k in 1:n_cores) {
        ...
        print("k=",k);
        print("k*col_per_shards",k*col_per_shards);
    ...
    for( n in 1:total_sample_size ) {//rows
          genotype_partials_asc[n]= identity_matrix(16);//to_matrix(e1);
      }
...
model{
  ...
  matrix[16,16] left;
  matrix[16,16] right;
  matrix[16,16] genotype_internal_partials_asc[total_sample_size]; //the last  columns are needed to correct for the ASC
