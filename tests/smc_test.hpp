...
class PosetSMCOnePopulationTest : public ::testing::TestWithParam<std::tuple<std::string, std::string, size_t,SMCOptions::ResamplingScheme, PosetSMC::PosetSMCKernel, double, double,double,  bool, bool  >> {
protected:
...
public:
    ...
    PosetSMCOnePopulationTest()
    {
        ...
        bool doPlots = std::get<8>(GetParam());
        ...
        bool isUltrametricTree = std::get<9>(GetParam());;
        
        char * newickWithoutHealthy;
        char * newickRooted;
        int count = 0;
        
        
        if (isUltrametricTree){
           initialRootedTree = pll_rtree_parse_newick(treefileName);
            if(!initialRootedTree){
                
                std::cout << "\n Error reading tree file:\n " << std::endl;
            }
            newickWithoutHealthy = pll_rtree_export_newick(initialRootedTree->root->left, NULL);
            ...
            pll_utree_t *utree = pll_utree_parse_newick(treefileName);
            if(!utree){
            ...
            pll_unode_t * root;
            for (size_t i = 0; i < utree->tip_count; ++i)
            {
              pll_unode_t * node = utree->nodes[i];
            
                if (node->label && std::string(node->label).compare(healthyTipLabel)==0){
            ...
            newickRooted = pll_utree_export_newick_rooted(root, 0.0);
            newickWithoutHealthy = pll_utree_export_newick(root, NULL);
            std::cout << "\n ML(Cellphy) tree newick: \n " << newickWithoutHealthy<< std::endl;
            initialRootedTree = pll_rtree_parse_newick_string(newickRooted);
            initialRootedTree = pll_rtree_parse_newick(newickRooted);
        ...
        double TscaledByTheta ;
        std::vector<double> deltas ;
        std::vector<double> timeOriginSTDs ;
        std::vector<std::vector<double>> coalTimesModelTimePerPopulation;
        
        true_theta =  std::get<5>(GetParam());
        deltas  = {std::get<6>(GetParam())};
        timeOriginSTDs = {std::get<7>(GetParam())};
